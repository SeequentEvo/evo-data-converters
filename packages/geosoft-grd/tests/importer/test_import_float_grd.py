...
def test_convert_grd_parser() -> None:
    ...
    with (
        patch(
            "evo.data_converters.grd.importer.grid_main.create_evo_object_service_and_data_client"
        ) as mock_create_client,
        patch(
            "evo.data_converters.grd.importer.array_to_parquet_parser.save_array_to_parquet"
...
def test_convert_color_grd_parser() -> None:
    ...
    with (
        ...
        assert result[0].origin == [0.0, 0.0, 0.0]
        assert result[0].size == [2500, 1768]
        assert result[0].cell_size == [1.0, 1.0]

        assert result[0].cell_attributes[0].name == "2d-grid-data-color"
        assert result[0].cell_attributes[0].attribute_type == "color"
        assert result[0].cell_attributes[0].key == "93774112608f9ff07abe76e6c0e32b422fe578e514daacb8a9ff31bc7adf392a"
        assert result[0].cell_attributes[0].values.length == 4420000
        assert result[0].cell_attributes[0].values.data_type == "uint32"
