...
async def export_omf_lineset(
    ...
    vertices_table = await data_client.download_table(
        object_id, version_id, linesegments_go.segments.vertices.as_dict()
    )
    ...
    segments_table = await data_client.download_table(object_id, version_id, linesegments_go.segments.indices.as_dict())
    segments = np.asarray(segments_table)

    segments_attribute_data = await export_omf_attributes(
        object_id, version_id, linesegments_go.segments.indices.attributes, "segments", data_client
    )

    if linesegments_go.parts:
        chunks_table = await data_client.download_table(object_id, version_id, linesegments_go.parts.chunks.as_dict())
