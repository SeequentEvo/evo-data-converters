...
from Deswik.Duf import DufImplementation, CompressionMethod, FilterCriteria
from Deswik.Entities import BaseEntity
from Deswik.Entities.Cad import Activator, Category, Upgrader, Polyface, Polyline
...
class DufWrapper:
    ...
    def LoadEntitiesInternal(self, layerGuid, dufGuidReferences):
        ...
        for entity in self._duf.LoadFromLatest(dufGuidReferences, Crit,
                                               False, True, None):
    ...
    def LoadDocInternal(self, dufGuidReferences):
        self.LoadTopLevelEntitiesOfType(Category.Document, dufGuidReferences, lambda item: self._doc.Loaded(Category.Document, item))
        self.LoadTopLevelEntitiesOfType(Category.LineTypes, dufGuidReferences, lambda item: self._doc.Loaded(Category.LineTypes, item))
        self.LoadTopLevelEntitiesOfType(Category.Images, dufGuidReferences, lambda item: self._doc.Loaded(Category.Images, item))
        self.LoadTopLevelEntitiesOfType(Category.Layers, dufGuidReferences, lambda item: self._doc.Loaded(Category.Layers, item))
        self.LoadTopLevelEntitiesOfType(Category.TextStyles, dufGuidReferences, lambda item: self._doc.Loaded(Category.TextStyles, item))
        self.LoadTopLevelEntitiesOfType(Category.DimStyles, dufGuidReferences, lambda item: self._doc.Loaded(Category.DimStyles, item))
        self.LoadTopLevelEntitiesOfType(Category.Blocks, dufGuidReferences, lambda item: self._doc.Loaded(Category.Blocks, item))
        self.LoadTopLevelEntitiesOfType(Category.HatchPatterns, dufGuidReferences, lambda item: self._doc.Loaded(Category.HatchPatterns, item))
        self.LoadTopLevelEntitiesOfType(Category.Lights, dufGuidReferences, lambda item: self._doc.Loaded(Category.Lights, item))
    ...
    def LoadSettings(self):
        ...
        zeroLayer = next(l for l in layers if l.Name == self.nameOfZeroLayer)
        ...
        settingsLayer = next((l for l in layers if l.Name == settingsLayerName), None)
...
class ObjectCollector:
    ...
    def get_objects_by_type(self, object_type: type) -> list[tuple[Category, BaseEntity]]:
        return [(cat, obj) for cat, cat_objs in self._objs.items() for klass, objs in cat_objs.items() for obj in objs if issubclass(klass, object_type)]
