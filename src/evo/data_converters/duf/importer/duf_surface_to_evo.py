...
def convert_duf_surface(
    ...
    indices_array = (
        np.fromiter(surface.FaceList, dtype=np.int32, count=num_faces * 5).reshape(
            num_faces, 5
        )[:, :3]
        - 1
    ...
    attributes = (
        [(xprop.Key, xprop.Value.Value[0].Value) for xprop in surface.XProperties]
        if surface.XProperties
        else []
    )
    ...
    mesh_vertices_go = Triangles_V1_2_0_Vertices(
        **data_client.save_table(vertices_table)
    )
    ...
    triangle_mesh_go = TriangleMesh_V2_1_0(
        ...
        triangles=Triangles_V1_2_0(
            vertices=mesh_vertices_go, indices=mesh_triangle_indices_go
        ),
        parts=mesh_parts_go,
    )
