...
def convert_duf_surface(
    ...
    indices_array = np.fromiter(
        surface.FaceList, dtype=np.int32, count=num_faces * 5
    ).reshape(num_faces, 5)[:, :3] - 1  # 1-indexed in the file, last element is -1 separator, 2nd last is same as 1st
    ...
    attributes = [(xprop.Key, xprop.Value.Value[0].Value) for xprop in surface.XProperties] if surface.XProperties else []
